...
from experiment_logger import Experiment, ExperimentRegistry
...
def network_visualization_component(experiment_name, epoch, height=200, gap=10, batch_gap=20, last_batch_scale=1.0, scroll_key=None):
    ...
    layer_to_gap = [
        (total_height_val - layer_to_num_images[layer_idx] * height) / (layer_to_num_images[layer_idx] + 1)
...
def load_weights_state(experiment_name, step):
    ...
    for m in range(len(model)):
        if not isinstance(model[m], nn.Linear):
            continue
...
def get_recommended_step_increment(experiment):
    if experiment.last_step < 50:
        return 1
    return experiment.last_step // 20


def load_weight_history(experiment_name, layer_idx, input_idx, output_idx):
    ex = Experiment(experiment_name, verbose=False)
    weight_history = []
...
def load_bias_history(experiment_name, layer_idx, output_idx):
    ex = Experiment(experiment_name, verbose=False)
    bias_history = []
    for epoch in range(0, ex.last_step+1, get_recommended_step_increment(ex)):
